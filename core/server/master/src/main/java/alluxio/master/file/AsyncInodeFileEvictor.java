...
final class AsyncInodeFileEvictor implements HeartbeatExecutor {
  ...
  private final long CHECKPOINT_INTERVAL = ServerConfiguration.getMs(PropertyKey.MASTER_INODE_CHECKPOINT_INTERVAL_MS);
  private final long SWEEP_BASE = ServerConfiguration.getLong(PropertyKey.MASTER_INODE_EVICT_BASE);
  ...
  private Map<Long, Long> mAccessTs;
  ...
  public AsyncInodeFileEvictor(FileSystemMaster fileSystemMaster, InodeTree inodeTree,
    ...
    mAccessTs = new ConcurrentHashMap<>();
    LOG.info("=== evict started, wait for 5 min to work");
  }

  public long getLastAccessTs(long id) {
    Long ts = mAccessTs.get(id);
    return (ts == null || ts < mMin) ? mMin : ts;
  }

  public void setLastAccessTs(long id, long ts) {
    if (mAccessTs.size() >= mInodeCapacity) {
      Iterator<Map.Entry<Long, Long>> it = mAccessTs.entrySet().iterator();
      for (int i = 0; i < 10 && it.hasNext(); i++) {
        mAccessTs.remove(it.next());
      }
    }
    mAccessTs.put(id, ts);
  }
  ...
  private void mark() {
    if (mInodeTree.estimateSize() <= mInodeCapacity) return;

    Iterator<MutableInode<?>> it = mInodeTree.iterator();
    mMin = Long.MAX_VALUE;
    mMax = Long.MIN_VALUE;
    while (it.hasNext()) {
      MutableInode node = it.next();
      long ts = mAccessTs.get(node.getId());
      if (node.isFile() && ts > 0) {
        mMax = Long.max(mMax, ts);
        mMin = Long.min(mMin, ts);
      }
    }
    if (mMax < mMin) mMax = mMin;
    mSweepIdx = 0;
    mStep = STEP_SWEEP;
    LOG.info("=== mark min {} max {} size {}", mMin, mMax, mInodeTree.estimateSize());
  }

  private void sweep() {
    long size = mInodeTree.estimateSize();
    ...
    Iterator<MutableInode<?>> it = mInodeTree.iterator();
    while (it.hasNext() && cnt < mSweepCnt && mInodeTree.estimateSize() > mInodeCapacityLow) {
      MutableInode node = it.next();
      try {
        if (node.isFile() && getLastAccessTs(node.getId()) < delTo) {
          AlluxioURI uri = mFileSystemMaster.getPath(node.getId());
          if (node.isPersisted()) {
            mFileSystemMaster.delete(uri, DeleteContext.mergeFrom(DeletePOptions.newBuilder().setAlluxioOnly(true)));
            mAccessTs.remove(node.getId());
        ...
        } else if (node.isDirectory() && getLastAccessTs(node.getId()) < delTo
            && node.asDirectory().getChildCount() == 0
            && !node.asDirectory().isMountPoint()) {
          AlluxioURI uri = mFileSystemMaster.getPath(node.getId());
          mFileSystemMaster.delete(uri, DeleteContext.mergeFrom(DeletePOptions.newBuilder().setAlluxioOnly(true)));
          mAccessTs.remove(node.getId());
          LOG.debug("=== evict async delete {}:{}", node.getId(), uri.getPath());
    ...
    LOG.info("=== evict sweep {} [{}]:{} -> {}", cnt, mSweepIdx, size, mInodeTree.estimateSize());

    if (cnt > 0 && size < mInodeTree.estimateSize()) {
  ...
  private boolean mayCheckPoint() throws IOException {
    ...
    while (it.hasNext()) {
      MutableInode node = it.next();
      if (node.isFile()) {
        idx = (int)((getLastAccessTs(node.getId()) - mMin) * 100 / (mMax - mMin));
        if (idx < 0) {
          idx = 0;
        } else if (idx >= slot100) {
          idx = slot100 - 1;
    ...
    low = mInodeTree.estimateSize() / 100 * 70;    // save up to 30% into checkpoint
    ...
    String dir = ServerConfiguration.get(PropertyKey.MASTER_JOURNAL_FOLDER);
    ...
    try (ObjectOutputStream os = new ObjectOutputStream(new FileOutputStream(src))) {
      ...
      while (it.hasNext()) {
        MutableInode node = it.next();
        if (node.isFile() && getLastAccessTs(node.getId()) >= low) {
          os.writeLong(node.getId());
          os.writeLong(getLastAccessTs(node.getId()));
          written++;
        }
      }
      LOG.info("=== do checkpoint {} out of {}", written, mInodeTree.estimateSize());
    }
  ...
  private void readCheckpoint() throws IOException {
    ...
    LOG.info("=== read checkpoint {} out of {}", loaded, mInodeTree.estimateSize());
    mStep = STEP_MARK;
  }
  ...
  public void heartbeat() {
    long wait = ServerConfiguration.getBoolean(PropertyKey.TEST_MODE) ? 30 * 1000 : 300 * 1000;
    if (System.currentTimeMillis() - mStartedWait < wait) return;

    wait = ServerConfiguration.getBoolean(PropertyKey.TEST_MODE) ? 60 * 1000 : 600 * 1000;
    if (System.currentTimeMillis() - mLastPrintTs >= wait) {
      LOG.info("=== evict size {}, sweep idx {}", mInodeTree.estimateSize(), mSweepIdx);
    ...
    try {
      ...
      switch (mStep) {
        case STEP_READ_CHECKPOINT:
          ...
          break;
        case STEP_MARK:
          if (mInodeTree.estimateSize() >= mInodeCapacity) {
            mark();
          }
          break;
        case STEP_SWEEP:
